...
`allureReport` формирует HTML-отчёт, `allureServe` открывает локальный просмотр. Аннотации добавляют сценариям название, категорию и приоритет. Автоматические скриншоты при падении пока не реализованы.
